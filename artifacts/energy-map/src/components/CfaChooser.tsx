...
import { CFA_SITES, CfaSite } from '@/data/cfaSites';
import { loadStickerPhotos } from '@/data/stickerLibrary';
import { panelInfoFor } from '@/data/panelInfo';

interface CfaChooserProps {
  onSelect: (site: CfaSite) => void;
  onClose: () => void;
}

// Site chooser for the City Football Academy — a vertical list of photo cards.
// Picking one opens that site's standard info panel.
export function CfaChooser({ onSelect, onClose }: CfaChooserProps) {
  const photos = loadStickerPhotos();

  return (
    <>
      <style>{`
        @keyframes cfa-chooser-in { from { opacity: 0; transform: translateY(100%); } to { opacity: 1; transform: translateY(0); } }
        @media (min-width: 640px) {
          @keyframes cfa-chooser-in { from { opacity: 0; transform: translateX(100%); } to { opacity: 1; transform: translateX(0); } }
        }
      `}</style>
      ...
      <div
        ...
        style={{ animation: 'cfa-chooser-in 0.28s cubic-bezier(0.16, 1, 0.3, 1)' }}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 shrink-0">
          <div>
            <h2 className="text-base font-bold text-gray-900 leading-tight">City Football Academy</h2>
            <p className="text-[11px] text-gray-400">Choose a site to view</p>
          </div>
          <button
            data-testid="cfa-chooser-close"
            onClick={onClose}
